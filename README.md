...
- 所有应用包括基座每次编译生产 8 位 `hash` 的入口文件，并生成 `import-map.json` 的映射文件
- 映射文件包含了 `file` 和 `name`两个字段，作为应用名和带有 `hash` 的路径
- 编译基座时收集所有应用的入口文件生成 `import-map` 资源表
- 根据基座编译时的 `hash` 构成资源表文件名
...
- 编译基座时，根据当前基座编译结果和 `dist` 下子应用的 `import-map` 映射文件生成 `imports` 资源表
- 设置基座和子应用的 `CI`，使子应用构建完成后自动向基座发起更新新的构建

> 使用 `CI` 构建代替 `HTTP` 请求的原因是，`HTTP` 请求有着极其不稳定的可能性

在 `webpack` 构建上（以下插件均来自 `@event-chat/micro-dev-config`）：
...
- 设置 `HtmlWebpackPlugin` 将构建哈希 `buildHash` 和环境变量 `DEPLOY_BASE` 提供给模板，匹配对应的 `hash` 资源表
- 为子应用编写插件 `CustomStandaloneDisabledPlugin` 设置单独访问拦截模板

> 通过本次更新发现 `single-spa` 的子应用是可以决定自身在基座的挂载位置以及样式，而基座更多是充当了路由导航的作用；这种情况很有可能会引发生产问题。
